...
import {
  ...
  messageToPayloadMessagesOpenAI,
} from "./utils";

const ChatComplitionPage = () => {
  ...
  const handleSendMessage = async (userMessage: string) => {
    ...
    let response:
      ...
      | null = null;
    const payloadMessages = messageToPayloadMessages(
      messages,
      numberOfMessages
    );
    const payloadMessagesOpenAI = messageToPayloadMessagesOpenAI(
      messages,
      numberOfMessages
    );
    try {
        ...
        response = await generateChatCompletionsOpenAI({
          ...
          onDownloadProgress: streamEnabled
            ? (pe: AxiosProgressEvent) => {
                ...
                // console.log(pe.event.currentTarget.responseText);
                const data = pe.event.currentTarget.responseText
                  ?.split("data: ")
                  .map((item: string) => {
    ...
    >
      <div className={styles.chatPage}>
        ...
        <ChatSettings
          ...
          onNumberOfMessagesChange={setNumberOfMessages}
        />
      </div>
    </PageContainer>
  );
...
export default ChatComplitionPage;
